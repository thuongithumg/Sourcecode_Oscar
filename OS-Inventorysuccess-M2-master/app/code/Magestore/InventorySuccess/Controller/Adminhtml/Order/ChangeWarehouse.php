...
class ChangeWarehouse extends \Magestore\InventorySuccess\Controller\Adminhtml\AbstractAction
{
    ...
    public function execute()
    {
        ...
        if (!$warehouseId) {
            $this->messageManager->addErrorMessage(__('Please select a warehouse!'));
            return $resultRedirect->setPath('sales/order/view', ['order_id' => $orderId]);
        }
        if (!$orderId) {
            $this->messageManager->addErrorMessage(__('Please select an order to change warehouse!'));
            return $resultRedirect->setPath('sales/order/index');
        }
        $warehouse = $this->_warehouseFactory->create();
        $warehouse->getResource()->load($warehouse, $warehouseId);
        if (!$warehouse->getId()) {
            $this->messageManager->addErrorMessage(__('Selected warehouse is not existed!'));
            return $resultRedirect->setPath('sales/order/view', ['order_id' => $orderId]);
        }
        try {
        ...
        if ($order->canShip()) {
            ...
            $this->messageManager->addSuccessMessage(__('Change warehouse for this order successfully!'));
        } else{
            $this->messageManager->addErrorMessage(__('Cannot change warehouse for this order!'));
        }
        return $resultRedirect->setPath('sales/order/view', ['order_id' => $orderId]);
    }
}