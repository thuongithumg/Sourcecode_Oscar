...
class Delete extends AbstractWarehouse
{
    ...
    public function execute()
    {
        ...
        if($product->getSumTotalQty()>0 || $product->getSumQtyToShip()>0){
            $this->messageManager->addErrorMessage(__('Can not delete this warehouse because it still contains some products'));
            return $resultRedirect->setPath('*/*/edit', ['id' => $id]);
        }
        try {
            $warehouse->getResource()->delete($warehouse);
            $this->messageManager->addSuccessMessage(__('Warehouse was successfully deleted'));
        }catch (\Exception $e){
