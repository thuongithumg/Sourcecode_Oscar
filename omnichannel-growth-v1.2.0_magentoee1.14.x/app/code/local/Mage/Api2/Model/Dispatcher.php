...
class Mage_Api2_Model_Dispatcher
{
    ...
    public function dispatch(Mage_Api2_Model_Request $request, Mage_Api2_Model_Response $response)
    {
        ...
        $currentSession = Mage::helper('webpos/permission')->getCurrentSession();
        $userType = ($currentSession ? 'admin' : $this->getApiUser()->getType());
        if($request->getPathInfo() == "/webpos/staff/login"){
            $userType = 'admin';
        }
        if ($userType != 'admin' && (strpos($request->getPathInfo(), 'webpos') !== false)
            && !(strpos($request->getPathInfo(), 'webpos/cart') !== false)) {
            throw new Mage_Api2_Exception('Not login', Mage_Api2_Model_Server::HTTP_UNAUTHORIZED);
        }
        ...
        if ($customerGroupId = $request->getParam('customer_group_id')) {

            Mage::getSingleton('customer/session')->setCustomerGroupId($customerGroupId);
            //todo: unset param customer_group_id to prevent filter
            $params = $request->getParams();
            $paramSource = $request->getParamSources();
            unset($params['customer_group_id']);
            $request->setParams($params);
            $request->setParamSources($paramSource);
