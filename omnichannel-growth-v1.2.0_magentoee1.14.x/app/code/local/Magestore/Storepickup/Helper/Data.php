...
/**
 * Class Magestore_Storepickup_Helper_Data
 */
...
//                    $newlistTime[] = $this->getSysTime($time['close_time_break']);
    ...
    public function getDefaultTimeChoose(){
        $data = Mage::getSingleton('checkout/session')->getData('storepickup_session');

        if(isset($data)){
            if(isset($data['time'])){
                $_time = str_replace(array(':00 AM',':00 PM'),array('',''),$data['time']);
                return '0'.$_time;
            } else {
                return null;
            }
    ...
    {
        $default_time = $this->getDefaultTimeChoose();
        // check shipping date
        ...
        if ($check_special) {
            ...
            if($default_time != null)
                $html .= '<option value="" >' . $this->__('Select Pickup Time') . '</option>';
            else
            ...
            if (count($listTime)) {
                foreach ($listTime as $value => $label) {
                    ...
                    if(strcasecmp($default_time,$value) == 0){
...
//    public function setImageBig($radio, $id = null) {
    ...
    public function getMarkerImages()
	{
		$result	= array (
			array (
				"height" => 53,
				"url" => Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_MEDIA). "storepickup/images/markerclusterer/m1.png",
				"width" => 53
			),
			array (
				"height" => 56,
				"url" => Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_MEDIA). "storepickup/images/markerclusterer/m2.png",
				"width" => 56
			),
			array (
				"height" => 66,
				"url" => Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_MEDIA). "storepickup/images/markerclusterer/m3.png",
				"width" => 66
			),
			array (
				"height" => 78,
				"url" => Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_MEDIA). "storepickup/images/markerclusterer/m4.png",
				"width" => 78
			),
			array (
				"height" => 90,
				"url" => Mage::getBaseUrl(Mage_Core_Model_Store::URL_TYPE_MEDIA). "storepickup/images/markerclusterer/m5.png",
				"width" => 90
			)
		);
