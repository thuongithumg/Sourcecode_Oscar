...
/**
 * Class Magestore_Storepickup_IndexController
 */
class Magestore_Storepickup_IndexController extends Mage_Core_Controller_Front_Action
{
    ...
    public function changestoreAction()
    {
        ...
        $before_data = Mage::getSingleton('checkout/session')->getData('storepickup_session');

        $data = array('store_id' => $storeId);
        if(isset($before_data)){
            if(isset($before_data['date']))
                $data['date'] = $before_data['date'];
            if(isset($before_data['time']))
                $data['time'] = $before_data['time'];
        }
