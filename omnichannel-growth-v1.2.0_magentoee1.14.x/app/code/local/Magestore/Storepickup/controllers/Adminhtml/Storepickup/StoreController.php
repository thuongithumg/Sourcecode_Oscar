...
/**
 * Class Magestore_Storepickup_Adminhtml_Storepickup_StoreController
 */
        ...
        if ($data = $this->getRequest()->getPost()) {
            ...
            try {
                ...
                foreach ($stores as $storeView) {
                    $model->setStoreId($storeView->getStoreId())
                            ->updateUrlKey();
                }
...
//
                ...
                if($model->getWarehouseId()){
                    Mage::getSingleton('adminhtml/session')->addError(Mage::helper('storepickup')->__('Unable to delete %s store',$model->getStoreName()));
                ...
                if ($i) {
                    Mage::getSingleton('adminhtml/session')->addSuccess(
                        Mage::helper('adminhtml')->__(
                            'Total of %d record(s) were successfully deleted', $i
                        )
                    );
                }
            } catch (Exception $e) {
        ...
        if (!is_array($storepickupIds)) {
            ...
            try {
                foreach ($storepickupIds as $storepickupId) {
                    Mage::getSingleton('storepickup/store')
                            ->load($storepickupId)
...
//                Mage::helper('storepickup')->exportAllStoreMultiViewToJson();
    ...
    public function validateStoreAction()
    {
        ...
        if (sizeof($collection) > 0) {
            ...
            foreach ($collection as $store) {
                if ($store->getData('state_id') > 0) {
                    $region = Mage::getModel('directory/region')->load($store->getData('state_id'));
                    if ($region->getId()) {
                        $store->setData('state', $region->getName())->save();
                    }
                } else {
                    if ($store->getData('state') != '') {
                        $state_id = Mage::helper('storepickup/region')->validateState($store->getData('country'), $store->getData('state'));

                        if ($state_id > Magestore_Storepickup_Helper_Region::STATE_ERROR)
                            $store->setData('state_id', $state_id)->save();
                        else {
                            $store->setData('status', 2)->save();
                            $error_message .= $state_id . '<br />' . $flag . ': ' . $store->getState() . ' of <strong>' . $store->getStoreName() . '</strong><br />';
                        }
                    } else {
                        $store->setData('status', 2)->save();
                        $error_message .= '<br />' . $flag . ': null of <strong>' . $store->getStoreName() . '</strong><br />';
                    }
                }
                $flag++;
            }
        ...
        if ($error_message != '') {
            $error_msg = 'The Stores invalid: ' . $error_message;
            Mage::getSingleton('core/session')->addNotice($error_msg);
        }

        Mage::getSingleton('core/session')->addSuccess('Validate Stores Successfully !');
