...
/**
 * Class Magestore_Storepickup_Block_Adminhtml_Store_Edit_Tab_Form
 */
class Magestore_Storepickup_Block_Adminhtml_Store_Edit_Tab_Form extends Mage_Adminhtml_Block_Widget_Form {
	...
	protected function _prepareForm() {
		...
		$array = array(array('label'=>'Select a warehouse','value'=>''));
        if (Mage::helper('core')->isModuleEnabled('Magestore_Inventorysuccess')){
            $warehouseOption = Mage::getModel('inventorysuccess/warehouse')->getCollection()->toOptionArray();
            foreach ($warehouseOption as $_option){
                $array[] = $_option;
            }
            $fieldset->addField('warehouse_id', 'select', array(
                'label' => Mage::helper('storepickup')->__('Link to warehouse'),
                'name' => 'warehouse_id',
                'values' => $array,
                'disabled' => true
            ));
