...
class Magestore_RewardPoints_Model_Mysql4_Setup extends Mage_Core_Model_Resource_Setup
{
    ...
    /**
     * update customer's reward balance from Enterprise Reward
     * 
     * @return Magestore_RewardPoints_Model_Mysql4_Setup
     */
    public function updateRewardBalanceFromEEReward()
    {
        if(Mage::helper('core')->isModuleEnabled('Enterprise_Reward')) {
            $customersReward = Mage::getModel('enterprise_reward/reward')->getCollection();
            $customersReward->getSelect()->join(array('customer_entity' =>
                $this->getTable('customer/entity')), 'main_table.customer_id = customer_entity.entity_id', array('customer_entity.*'));
            Mage::getResourceModel('rewardpoints/transaction')->updateEnterpriseReward($customersReward);
        }
        return $this;
    }
}
