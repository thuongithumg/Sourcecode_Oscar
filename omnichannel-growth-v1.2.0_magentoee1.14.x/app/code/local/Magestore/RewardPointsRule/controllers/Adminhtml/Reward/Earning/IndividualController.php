 ...
 */
class Magestore_RewardPointsRule_Adminhtml_Reward_Earning_IndividualController extends Mage_Adminhtml_Controller_Action {


    protected function _isAllowed()
    {
        return Mage::getSingleton('admin/session')->isAllowed('rewardpoints/earning/individual');
    }
...
//        $this->_addLeft($this->getLayout()->createBlock('rewardpointsrule/adminhtml_earning_individual_edit_tabs'));
//        $this->_addContent($this->getLayout()->createBlock('rewardpointsrule/adminhtml_earning_individual_edit'));
        $this->_addContent($this->getLayout()->createBlock('rewardpointsrule/adminhtml_earning_individual'));
        $this->renderLayout();
    }
    ...
    public function gridAction() {
        ...
        $this->getResponse()->setBody(
                $this->getLayout()
                        ->createBlock('rewardpointsrule/adminhtml_earning_individual_grid')
                        ->toHtml()
        );
    }
            ...
            if($point!='' && (!is_numeric($point) || $point<0)){
                ...
                try {
                    ...
                    if ($product && $product->getId()) {
                        $product->setRewardpointsEarn($point)
                                ->save();
                        $out['value'] = Mage::getModel('catalog/product')->load($id)->getRewardpointsEarn();
                    } else {
    ...
    public function massChangePointAction(){
        ...
        if($point!='' && (!is_numeric($point) || $point < 0)){
            ...
            try {
                Mage::getResourceModel('catalog/product_action')->updateAttributes($productIds, array('rewardpoints_earn' => $point), 0);
                Mage::getSingleton('adminhtml/session')->addSuccess(
                    Mage::helper('rewardpointsrule')->__('Update earning point successfully.')
                );
            } catch (Exception $e) {
