...
class Magestore_Customercredit_Model_Observer
{
    ...
    public function catalogProductSaveAfter($observer)
    {
        ...
        if ($product->getTypeId() == "customercredit" && trim($product->getCreditRate()) == "") {
            $defaultValue = $product->getResource()->getAttribute('credit_rate')->getDefaultValue();
            $action = Mage::getModel('catalog/resource_product_action');
            $action->updateAttributes(array($product_id), array(
                'credit_rate' => $defaultValue
            ), 0);
        }
    }
...
//            $total_customercredit_invoiced = $data['total_customercredit_invoiced'];
    ...
    public function invoiceSaveAfter($observer)
    {
        ...
        foreach ($invoice->getAllItems() as $item) {
            if (!$item->getParentItemId()) {
                ...
                if (!isset($buyRequest['amount']) && isset($creditAmount['type']) && $creditAmount['type'] == 'static') {
                    $amount = $this->convertPrice($product, $creditAmount['value']);
                    $buyRequest['amount'] = $amount;
                }

                if ($type == 'customercredit') {
                    if (isset($buyRequest['send_friend']) && isset($buyRequest['recipient_email']) && $buyRequest['send_friend']
    ...
    public function creditmemoSaveAfter(Varien_Event_Observer $observer)
    {
        ...
        if ($amount_credit) {
            ...
            try {
                ...
                return Mage::helper('customercredit')->__($e->getMessage());
            }
        }
    }
    ...
    public function isAllowedGuestCheckout(Varien_Event_Observer $observer)
    {
        ...
        $session->setHasCustomerCreditItem($isContain);
