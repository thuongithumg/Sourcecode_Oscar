 ...
 */
class Magestore_Inventorysuccess_Block_Adminhtml_Transferstock_Sendstock_Edit_Tab_Receiving
    extends
    Mage_Adminhtml_Block_Widget_Grid
{
        ...
        $this->setChild('import',
                        $this->getLayout()->createBlock('adminhtml/widget_button')
                             ->setData(array(
                                           ...
                                           'onclick' => "jQuery('#import-send-receiving').modal();",
                                           'class'   => 'import-csv',
                                       ))
        );

        $handleBarcodeUrl = Mage::helper('adminhtml')->getUrl(
            'adminhtml/inventorysuccess_transferstock_sendstock/handleBarcodeReceiving',
    ...
    protected function _prepareCollection()
    {
        ...
        $collection->getSelect()->where('(qty - qty_received - qty_returned) > ? ', 0);
    ...
    public function getSelectedProducts()
    {
        ...
        if ( !$this->_selectedProducts ) {
            $data = array();
            if ( Mage::getSingleton('adminhtml/session')->getData('send_receiving_products') ) {
                $data = Mage::getSingleton('adminhtml/session')->getData('send_receiving_products');
            }
