 ...
 */
class Magestore_Inventorysuccess_Block_Adminhtml_Transferstock_Requeststock_Edit_Tab_General
    extends
    Mage_Adminhtml_Block_Widget_Form
{
     ...
     *
     * @return Magestore_Inventorysuccess_Block_Adminhtml_Requeststock_Edit_Tab_General
     */
    protected function _prepareForm()
    {
        ...
        $data = $this->_prepareDefaultValue($data);
        ...
        $resourceId = 'admin/inventorysuccess/create_transferstock/create_requeststock';
     ...
     * @return array
     */
    protected function _prepareDefaultValue( $data )
    {
        if ( !isset($data['transferstock_id']) || !$data['transferstock_id'] ) {
            if ( $orderId = $this->getRequest()->getParam('order_id') ) {
                $data['reason'] = $this->__('Request stock to create shipment for order #%s', $orderId);
            }
            if ( $desWarehouseId = $this->getRequest()->getParam('des_warehouse_id') ) {
                $data['des_warehouse_id'] = $desWarehouseId;
            }
        }
        return $data;
    }
    ...
    protected function getStepIndex()
    {
        ...
        $transfer = Mage::registry('requeststock_data');
