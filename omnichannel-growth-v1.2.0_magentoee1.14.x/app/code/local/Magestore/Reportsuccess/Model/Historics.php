    ...
    extends Varien_Object
{
    ...
    {
        $backupData = Mage::helper('reportsuccess/backup')->extractDataFromFilename($fileName);

        $this->addData(array(
            'id'   => $filePath . DS . $fileName,
            'time' => (int)$backupData->getTime(),
            'path' => $filePath,
            'extension' => Mage::helper('reportsuccess/backup')->getExtensionByType($backupData->getType()),
            'display_name' => Mage::helper('reportsuccess/backup')->nameToDisplayName($backupData->getName()),
    ...
    public function getFileName()
    {
        ...
        $filename .= '.' . Mage::helper('reportsuccess/backup')->getExtensionByType($this->getType());
    ...
    {
        $possibleTypes = Mage::helper('reportsuccess/backup')->getBackupTypesList();
        if(!in_array($value, $possibleTypes)) {
            $value = Mage::helper('reportsuccess/backup')->getDefaultBackupType();
        }
    ...
    {
        $backupsCollection = Mage::getSingleton('reportsuccess/fs_collection');
