 ...
 */
class Magestore_Purchaseordersuccess_Block_Adminhtml_Purchaseorder_Edit_Tab_Returneditem_Grid
    extends Magestore_Purchaseordersuccess_Block_Adminhtml_Abstractgrid
{
    ...
    {
        parent::__construct();
        $this->setId('purchase_order_returned_item_list');
        $this->setDefaultSort('returned_at');
    ...
    {
        /** @var Magestore_Purchaseordersuccess_Model_Mysql4_Purchaseorder_Item_Received_Collection $collection */
        $ids = $this->purchaseOrder->getItems()->getAllIds();
        $collection = Mage::getResourceModel('purchaseordersuccess/purchaseorder_item_returned_collection')
    ...
    /**
     * Prepare Returned item grid columns
     *
     * @return $this
     * @throws Exception
     */
    ...
    {
        $this->addColumn('returned_at',
            array(
                'header' => $this->__('Return Date'),
                'align' => 'left',
                'index' => 'returned_at',
                'type' => 'date',
        ...
        $this->addExportType('*/purchaseordersuccess_purchaseorder_returneditem/exportCsv', $this->__('CSV'));
        $this->addExportType('*/purchaseordersuccess_purchaseorder_returneditem/exportXml', $this->__('Excel XML'));
    ...
    {
        return $this->getUrl(
            "*/purchaseordersuccess_purchaseorder_returneditem/grid",
    ...
    protected function _filterQtyCallback($collection, $column)
    {
        ...
        if (isset($value['from']))
            $collection->addFieldToFilter('main_table.qty_returned', array('gteq' => $value['from']));
        if (isset($value['to']))
            $collection->addFieldToFilter('main_table.qty_returned', array('lteq' => $value['to']));
        return $collection;
    }
}