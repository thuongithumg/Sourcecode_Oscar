...
class Magestore_Purchaseordersuccess_Adminhtml_Purchaseordersuccess_Purchaseorder_Returneditem_ImportController
    extends Magestore_Purchaseordersuccess_Controller_Action
{
    ...
    public function downloadSampleAction()
    {
        ...
        $fileName = 'purchase_order_returned_item_import_sample.csv';
        $content = $this->importService->getSampleReturnedImportCSV($purchaseId);
    ...
    public function importAction()
    {
        if ($this->getRequest()->isPost()) {
            if (isset($_FILES['import_product']['name']) && $_FILES['import_product']['name'] != '') {
                ...
                if ($extension != 'csv') {
                ...
                } else if(!isset($params['returned_at'])){
                    $this->getAdminSession()->addError($this->__('Please select Date to Return'));
                }
                else{
                    ...
                    $purchaseId = $this->getRequest()->getParam('id');
                    $returnedAt = $params['returned_at'];
                    $validateData = $this->returnedService->validateReturnedProductImported($dataImported, $purchaseId);
                    ...
                    if (!empty($selectedItems)) {
                        ...
                        if (empty($returnedData)) {
                            ...
                            $total_returned_item = $returnedData['total_returned_item'];
                            try {
                                $this->purchaseorderService->returnProducts($purchaseId, $returnedData, $returnedAt);
                                $this->getAdminSession()->addSuccess($this->__('Return '.$total_returned_item.' item(s) successfully.'));
                            } catch (\Exception $e) {
