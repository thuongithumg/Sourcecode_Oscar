...
class Magestore_Webpos_Model_Api2_Session_Rest_Admin_V1 extends Magestore_Webpos_Model_Api2_Abstract implements Magestore_Webpos_Api_SessionInterface
{
    ...
    public function __construct() {
        $this->_service = $this->_createService('session_session');
        $this->_transactionService = $this->_createService('session_session');
    ...
    public function dispatch()
    {
        ...
        switch ($this->getActionType()) {
            case self::ACTION_GET_LIST:
                $params = $this->_processRequestParams('filter');
                $postId =  $params[0]['eq'];
                $result = $this->_service->getSessionData($postId);
                $this->_render($result);
                $this->getResponse()->setHttpResponseCode(Mage_Api2_Model_Server::HTTP_OK);
                break;
            case self::ACTION_CLOSE_SESSION:
            ...
            case self::ACTION_SAVE:
                $zReportData = $this->_processRequestParams('shift');
