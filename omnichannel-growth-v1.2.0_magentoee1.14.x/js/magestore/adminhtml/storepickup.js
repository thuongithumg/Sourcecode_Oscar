...
function updateStoreShippingPrice(price) {
    var s_method = 'order-shipping-method-info';
    if ($(s_method) && $(s_method).down('.price')) {
        $(s_method).down('.price').update(price);
    }
...
{
        if ($('shipping_time').options.length == 0){
            var response = JSON.parse($('shipping_time').innerHTML);
